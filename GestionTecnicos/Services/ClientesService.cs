...
public class ClientesService(IDbContextFactory<Contexto> DbFactory)
{
    ...
    {
        await using var Contexto = await DbFactory.CreateDbContextAsync();
        return await Contexto.Clientes.Include(c => c.Tecnico)
            .Include(c => c.Ciudad)
    ...
    public async Task<bool> ExisteNombre(int cliente, string nombre)
    {
        ...
        return await Contexto.Clientes.AnyAsync(c =>
            c.ClienteId != cliente && c.Nombres.ToLower().Equals(nombre.ToLower()));
    }
    ...
    public async Task<bool> ExisteRnc(int ClienteId, string rnc)
    {
        await using var Contexto = await DbFactory.CreateDbContextAsync();
        return await Contexto.Clientes.AnyAsync(c =>
            c.ClienteId != ClienteId && c.Nombres.ToLower().Equals(rnc.ToLower()));
    }
